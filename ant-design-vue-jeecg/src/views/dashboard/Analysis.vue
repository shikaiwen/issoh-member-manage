  ...
  <page-layout :avatar="avatar">
    ...
    <div slot="extra">
      <a-row class="more-info">
        <a-col :span="8">
          <head-info title="案件数" content="56" :center="false" :bordered="false"/>
        </a-col>
        <a-col :span="8">
          <head-info title="社員数" content="8/24" :center="false" :bordered="false"/>
        </a-col>
        <a-col :span="8">
    ...
    <div>
      <a-row :gutter="24">
        <a-col :xl="16" :lg="24" :md="24" :sm="24" :xs="24">
          <a-card :loading="loading" title="メッセージ" :bordered="false">
            <a-list>
              <a-list-item :key="index" v-for="(item, index) in activities">
                <a-list-item-meta>
                  <!-- <a-avatar slot="avatar" :src="item.user.avatar" /> -->
                  <div slot="title">
                    <span>{{ item.name }}さんは</span>&nbsp;
                    入場してる「{{ item.projectName }}」案件は{{item.endTime}}に終了する予定です。
                    <!-- <span>{{ item.project.action }}</span>&nbsp; -->
                    <!-- <a href="#"></a> -->
                    <router-link :to="`/projects?name=${item.projectName}`">案件情報へ</router-link>
                  </div>
                  <div slot="description">{{ item.endTime }}</div>
                </a-list-item-meta>
              </a-list-item>
  ...
  export default {
    ...
    methods: {
      ...
      getActivity() {
        ...
        var porjects = [
          {name:"施凱文",projectName:"社内案件",endTime:"2020/11/30"},
          {name:"龍介",projectName:"通信会社Web開発案件",endTime:"2020/11/30"},
        ]
        this.activities = porjects;
